...
* Inline assembly with no corresponding aarch64/sw64 inlne assembly.
* Asssembly source files with no corresponding aarch64/sw64 assembly source files.
* Missing aarch64/sw64 architecture detection in autoconf config.guess scripts.
* Linking against libraries that are not available on the aarch64/sw64 architecture.
* Use of architecture specific intrinsics.
* Preprocessor errors that trigger when compiling on aarch64/sw64.
...
Produces an aarch64/sw64 porting readiness report.
...
optional arguments:
  ...
  --arch ARCH           Instruction set schema name (armv8.6-a+sve2, arm64 or sw64, 
                        default: armv8.6-a+sve2,)
...
Available issue types:
  ...
  PragmaSimd, PreprocessorError, SW64

```

# Hints:
- CompilerSpecificIssue is not supported by the scanner now.
- armv8.6-a+sve2 parameter indicates target arch is yitian and target os is with linux.
