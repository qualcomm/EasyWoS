...
description: |
  ...
  2. **ETL file** (first follow the `perf-sampling-parser` skill instructions
     in the current agent context to analyze per-process CPU usage; the user
     picks the target process and exports a SpeedScope JSON, then proceeds to
     Phase 1)
  ...
  **The target process is NOT specified up front by the user**: when the input
  ...
  following the perf-sampling-parser workflow. The target process must be chosen
  by the user based on real data — the process tree and Top-N CPU breakdown
  produced by perf-sampling-parser.
...
1. **Phase 0 (optional) ETL → SpeedScope JSON** — when the input is an `.etl`,
   load and follow `skills/perf-sampling-parser/SKILL.md` in the current agent
   context to do the parsing; the target process is selected by the user after
...
The commands below are shell-neutral Python invocations. Examples that use
line continuations or temporary-file creation may be written in bash form; on
PowerShell, use PowerShell quoting/continuations and `Set-Content` or
here-strings for temporary JSON files.
...
| `*.etl` or `*.etl.zip` | **Must run Step 0a first**: follow `skills/perf-sampling-parser/SKILL.md` in-context to convert the ETL to a SpeedScope JSON |

**Step 0a — Follow perf-sampling-parser to handle the ETL (only when the input is an ETL)**

Do not try to parse the ETL yourself or run xperf / WPA. Codex does not have a
generic "Skill tool" call. Instead, read `skills/perf-sampling-parser/SKILL.md`
and execute that workflow in the current context using the user-provided ETL
path as the input.
