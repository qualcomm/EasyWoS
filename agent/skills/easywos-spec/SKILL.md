...
### 0.3 If any dependency is missing - report first, install only with approval

If one or more dependencies are not found, **do not proceed** with parsing,
screening, matching, or task generation. Instead:

1. **Handle generated data separately.** If the only missing dependency is
   `skills/combined-spec-summary.yaml` and `dispatcher-skill` is present,
   regenerate it with:
   ...
   Then re-run the 0.2 check.

2. **For missing skill directories or scripts, stop and report first.** Do not
   automatically install packages or run network-backed commands. List the
   complete missing set and ask the user whether to install from the canonical
   repo.
...
4. **Re-run the 0.2 check** after installation completes.

5. **If dependencies are STILL missing**, stop and report the complete list of
   ...
   Approve `npx skills add qualcomm/EasyWoS/agent --all`, or locate/install
   these dependent skills manually, then re-invoke easywos-spec.
   ```
...
> `node skills/dispatcher-skill/scripts/combine-specs.js`; do not ask the user
> to reinstall skills just to recreate this generated file.
...
> **Harness compatibility.** The `/easywos-spec …` form is the Claude Code
> slash-command shorthand. This skill is harness-agnostic: on **Codex** (and any
> harness without slash-command skills) there is no `/command` — trigger it by
> describing the task in natural language and giving the same YAML path, e.g.
> *"Use the easywos-spec skill to match `<path-to-easywos-output.yaml>` to specs."*
> The `/dispatcher-skill …` commands this skill emits into `tasks.md` (§5) are
> likewise the Claude shorthand; on Codex, treat each such line as an instruction
> to invoke the dispatcher-skill with those exact arguments. Only the surface
> syntax differs.
