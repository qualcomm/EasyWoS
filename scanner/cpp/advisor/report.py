...
class Report:
    ...
    def write(self, output_file, report_errors=False, report_remarks=False, include_summary=False):
        ...
        if include_summary:
            items[ReportItem.SUMMARY.get('type')].append(
                FilesScannedRemark(len(self.source_files)))

            issue_type_except = [ReportItem.SUMMARY.get('type'),
                                 ReportItem.ERROR.get('type')]
            if not sum([len(items[type.get('type')]) for type in ReportItem.TYPES if
                        type.get('type') not in issue_type_except]) and report_remarks:
                items[ReportItem.NO_ISSUES_FOUND_REMARK.get('type')].append(NoIssuesFoundRemark())
